...
//		case 5: set .@map$,"arug_cas05"; set .@x,321; set .@y,153; break;
		case 1: set .@map$,"aru_gld"; set .@x,265; set .@y,22; break;
		case 2: set .@map$,"aru_gld"; set .@x,265; set .@y,22; break;
		case 3: set .@map$,"aru_gld"; set .@x,265; set .@y,22; break;
		case 4: set .@map$,"aru_gld"; set .@x,265; set .@y,22; break;
		case 5: set .@map$,"aru_gld"; set .@x,265; set .@y,22; break;
		}
	}
	else if(.@tag$ == "N") {
		switch(.@num) {
...
//		case 5: set .@map$,"schg_cas05"; set .@x,369; set .@y,306; break;
		case 1: set .@map$,"sch_gld"; set .@x,288; set .@y,229; break;
		case 2: set .@map$,"sch_gld"; set .@x,288; set .@y,229; break;
		case 3: set .@map$,"sch_gld"; set .@x,288; set .@y,229;  break;
		case 4: set .@map$,"sch_gld"; set .@x,288; set .@y,229; break;
		case 5: set .@map$,"sch_gld"; set .@x,288; set .@y,229; break;
		}
	}
...
//	set .@map$,"que_q" +.@name$+ "";
	set .@ini$, substr2(strnpcinfo(2), 0, 1);
	set .@num$, substr2(strnpcinfo(2), 1, 1);
	set .@map$, "que_q";
	if (.@ini$ == "F") set .@map$, .@map$ + "aru";
	else set .@map$, .@map$ + "sch";
	set .@map$, .@map$ + "0" + .@num$;
	if($GodSEActivete[dupvar(.castle)] == 0) {
		mes "[案内人]";
//		mes "この砦には隠された秘密が多いです。";
//		mes "その中のひとつが、";
		mes "このダンジョンには隠された秘密が";
		mes "多いです。その中のひとつが、";
...
//				mes "砦をもっと発展させなければ……";
	...
	else if($GodSEActivete[dupvar(.castle)] == 1) {
		mes "[案内人]";
		mes "悠長に会話をしている";
		mes "余裕はありませんね。";
		mes "幸運をお祈りします。";
...
//	else if($GodSEActivete[dupvar(.castle)] >= 2 && $GodSEActivete[dupvar(.castle)] < 14) {
	else if($GodSEActivete[dupvar(.castle)] >= 2 && $GodSEActivete[dupvar(.castle)] < 4) {
		mes "[案内人]";
		mes "オーコルニルが再構築されるまでには";
//		mes "多くの時間を要すると言われています。";
		mes "少しの時間を要すると言われています。";
		mes "例えそれが実在しない";
		mes "空間だとしても……です。";
		next;
		mes "[案内人]";
...
OnInit:
	...
	if(.@name$ == "F")
		...
		set dupvar(.castle),5;
	if(.@num > 0)
		set dupvar(.castle),dupvar(.castle)+.@num-1;
	if($GodSEActivete[dupvar(.castle)] == 1)
		set $GodSEActivete[dupvar(.castle)],0;
	end;
}
...
arug_dun01,200,196,4	duplicate(案内人#Okolnir)	案内人#F1	899
schg_dun01,200,171,4	duplicate(案内人#Okolnir)	案内人#N1	899
...
//	set .@GID,getcastledata(.@name$+"g_cas0"+.@num+"",1);
//	if(getcharid(2) != .@GID) {
...
//		countitem(969) >= 10 && countitem(757) >= 20)
		countitem(969) >= 10 && countitem(985) >= 20)
	) {
...
//				if(.@saram > 15 && .@saram < 21) {
					...
					mes "我のもとに到達せよ！";
					close2;
					if (dupvar(.nic$, "okolnir#" + substr2(strnpcinfo(2), 0, 2)) == "") {
						set dupvar(.nic$, "okolnir#" + substr2(strnpcinfo(2), 0, 2)), strcharinfo(0);
...
//						announce "["+ GetGuildName(.@GID) +"]ギルドの["+ strcharinfo(0) +"]様により、虚像のオーコルニルが開放されました。",0;
						announce "["+ strcharinfo(0) +"]により、虚像のオーコルニルが開放されました。",0;
						hideonnpc;
					}
//				}
//				else {
...
OnDisable:
	...
	hideonnpc "windpath03#"+.@name$;
	hideonnpc "windpath04#"+.@name$;
	set dupvar(.gqse_mobcount,"Gate_manager#"+ .@name$),0;
	set dupvar(.gqse_pccount,"Gate_manager#"+ .@name$),0;
	set dupvar(.gqse_cagecount,"gdtimer01#"+ .@name$),0;
	set dupvar(.gqse_sealcount,"ウィッシュメイデン#"+ .@name$ +"_b"),0;
	stopnpctimer;
	set dupvar(.nic$), "";
	end;
OnStop:
...
//		close;
//	}
//	set .@miro,dupvar(.gqse_mobcount,"Gate_manager#"+strnpcinfo(2));
//	if(.@miro == dupvar(.gqse_pccount,"Gate_manager#"+strnpcinfo(2))) {
		switch(rand(1,5)) {
...
//	case Job_Magician:     set .@mob,1663; break;
...
//	set dupvar(.gqse_mobcount,"Gate_manager#"+strnpcinfo(2)),.@n[4];
//	warp .@map$,.@n[2],.@n[3];
	end;
...
OnStart:
	...
	for (set .@i, 0; .@i < 18; ++.@i)
		monster 
			...
			.@mob_ids[.@i % 6],
			1,
			"Gate_manager#" + strnpcinfo(2) + "::OnKilled";
	end;
OnInit:
	set .@dummy,getmapxy(.@map$,.@dummy,.@dummy,1);		//.@map$以外はダミー
	if(.@map$ == "-")					//duplicate元は何もしない
...
OnStart:
	set dupvar(.gqse_mobcount),0;
	set dupvar(.gqse_pccount),getmapusers("this");
	end;
OnKilled:
//	if(dupvar(.gqse_mobcount) == dupvar(.gqse_pccount)) {
...
//	sc_start SC_STONE,300000,0,10000;
	switch(dupvar(.gqse_cagecount,"gdtimer01#"+ .@name$)) {
	...
	hideonnpc;
//	warp .@map$,.@n$[0],.@n$[1];
	set dupvar(.gqse_cagecount,"gdtimer01#"+ .@name$),.@n$[2];
...
OnKilled1:
	...
	if(getmapmobs("this","nmsom_jin01#"+.@name$+"::OnKilled1") == 0) {
		set dupvar(.gqse_sealcount,"ウィッシュメイデン#"+ .@name$ +"_b"),dupvar(.gqse_sealcount,"ウィッシュメイデン#"+ .@name$ +"_b")+1;
		donpcevent "nmsom_jin01#"+.@name$+"::OnDisable";
		if(dupvar(.gqse_sealcount,"ウィッシュメイデン#"+ .@name$ +"_b") == 3) {
...
//	if(getcharid(2) == .@GID) {
		//if(strcharinfo(0) != getguildmaster(.@GID)) {
		if (strcharinfo(0) != dupvar(.nic$, "okolnir#" + substr2(strnpcinfo(2), 0, 2))) {
			cutin "wish_maiden12",1;
			mes "[ウィッシュメイデン]";
...
//			setarray .@need,7830,7831,7832,7833,7834,2357,7510,969,757;
			setarray .@need,7830,7831,7832,7833,7834,2357,7510,969,985;
...
que_qaru01,252,339,0	warp	to_agit_gate#F1	1,1,aru_gld,213,199
que_qaru02,252,339,0	warp	to_agit_gate#F2	1,1,aru_gld,213,199
que_qaru03,252,339,0	warp	to_agit_gate#F3	1,1,aru_gld,213,199
que_qaru04,252,339,0	warp	to_agit_gate#F4	1,1,aru_gld,213,199
que_qaru05,252,339,0	warp	to_agit_gate#F5	1,1,aru_gld,213,199
que_qsch01,252,339,0	warp	to_agit_gate#N1	1,1,sch_gld,218,83
que_qsch02,252,339,0	warp	to_agit_gate#N2	1,1,sch_gld,218,83
que_qsch03,252,339,0	warp	to_agit_gate#N3	1,1,sch_gld,218,83
que_qsch04,252,339,0	warp	to_agit_gate#N4	1,1,sch_gld,218,83
que_qsch05,252,339,0	warp	to_agit_gate#N5	1,1,sch_gld,218,83

-	script	okolnir_timer#Okolnir	-1,{
OnEnable:
	initnpctimer;
	set $GodSEActivete[dupvar(.castle)],2;
	end;
OnReset:
	set $GodSEActivete[dupvar(.castle)],0;
	stopnpctimer;
...
OnTimer3600000:
//	if($GodSEActivete[dupvar(.castle)] < 14) {
	if($GodSEActivete[dupvar(.castle)] < 4) {
		set $GodSEActivete[dupvar(.castle)],$GodSEActivete[dupvar(.castle)]+1;
		initnpctimer;
	}
//	else if($GodSEActivete[dupvar(.castle)] == 14) {
	else if($GodSEActivete[dupvar(.castle)] >= 4) {
		set $GodSEActivete[dupvar(.castle)],0;
		set .@name$,strnpcinfo(2);
...
OnInit:
	...
	if(.@num > 0)
		set dupvar(.castle),dupvar(.castle)+.@num-1;
	if($GodSEActivete[dupvar(.castle)] >= 2)
		initnpctimer;
	end;
}
