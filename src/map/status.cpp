...
void initChangeTables(void)
{
	...
	// [GonBee]
	// リコグナイズドスペルのアイコンを爆裂波動のアイコンとして使用する。
	//set_sc( MO_EXPLOSIONSPIRITS	, SC_EXPLOSIONSPIRITS	, EFST_EXPLOSIONSPIRITS	, SCB_CRI|SCB_REGEN );
	set_sc( MO_EXPLOSIONSPIRITS	, SC_EXPLOSIONSPIRITS	, EFST_RECOGNIZEDSPELL	, SCB_CRI|SCB_REGEN );
...
int status_calc_pc_sub(struct map_session_data* sd, enum e_status_calc_opt opt)
{
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory_data[index]->type == IT_WEAPON) {
			...
			// [GonBee]
			// 靴の精錬は素手のTaekwon KidとStar GladiatorならLv4武器の精錬と同じ効果がある。
			if (r &&
...
// ----- EQUIPMENT-MDEF CALCULATION -----
	...
	// [GonBee]
	// Mdefの制限はstatus_calc_bl_mainで行う。
