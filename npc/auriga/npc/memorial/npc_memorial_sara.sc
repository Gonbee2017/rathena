...
dali,130,107,5	script	探検家レーン#sara	945,{
	if((Job <= Job_Dancer && Upper != UPPER_HIGH) ||
		...
		mes "　限界突破したスーパーノービス、";
		mes "　影狼、朧のBaseLv85以上^000000になったら";
		mes "　また来よう‐";
		close;
	}
	...
	case 0:
		if(checkquest2(15002)) {
			...
			if(!(checkquest2(15002) & 0x2)) {
				...
				mes "[探検家レーン]";
				mes "歪んだ次元が元に戻るまでには";
				mes "^FF0000最後に入ってから20時間必要だ。^000000";
				mes "時間が経過したら、また来てくれ！";
				close;
			}
			mes "[探検家レーン]";
	...
	case 8:
		if(SARA_1QUE == 0) {	// 初回
			...
			mes "ある程度の時間がかかるのだ。";
			mes "^FF0000最後に入ってから20時間以内は";
			mes "再度入ることが出来ない。^000000";
			mes "気をつけてくれ。";
		}
		else {
			mes "[探検家レーン]";
		...
		delquest 15003;
		set SARA_1QUE,1;
		if(Job >= Job_RuneKnight && Job <= Job_Oboro) {
			getexp 500000,0;
			getexp 500000,0;
			getexp 0,500000;
		}
...
dali,139,118,3	script	次元移動機#sara	10007,{
	...
	if(checkquest2(15002)) {
		mes "^ff0000‐歪んだ次元が元に戻るまで";
		mes "　20時間かかります。";
		mes "　それまで再入場が不可能になります‐^000000";
		close;
	}
